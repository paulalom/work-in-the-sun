...
function codexUiProjectLabel(target: JsonRecord = {}) {
  for (const value of [target.route, target.sessionHint, target.label]) {
    ...
    if (!text || /^new$/i.test(text) || /^current$/i.test(text)) {
      continue;
    }
...
async function runUiBridge(action: string, options: JsonRecord = {}, runOptions: JsonRecord = {}) {
  ...
  if (options.projectLabel) {
    args.push("-ProjectLabel", options.projectLabel);
  }

  if (options.projectWorkspace) {
    args.push("-Workspace", options.projectWorkspace);
  }
...
async function dispatchUiNow(command, route) {
  ...
  const sendResult = await runUiBridge("send", {
    ...
    projectWorkspace: route.projectWorkspace,
