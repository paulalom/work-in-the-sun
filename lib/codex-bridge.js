...
function dispatchRoute(command) {
  ...
  if (DELIVERY_MODE === WINDOWS_UI_MODE) {
    ...
    const label = target.route || target.label || target.sessionHint || "";

    return {
      accepted: true,
      mode: WINDOWS_UI_MODE,
      targetLabel: /^current$/i.test(label) ? "" : label,
    };
  }
...
async function runUiBridge(action, options = {}) {
  const args = [
    "-STA",
    "-NoProfile",
    "-ExecutionPolicy",
  ...
  try {
    const result = await runPowerShell(args, { cwd: path.join(__dirname, "..") });
...
async function dispatchUiNow(command, route) {
  ...
  const result = await runUiBridge("send", {
    text: command.text,
  ...
  await appendBridgeEvent(command, {
    level: "system",
    text: `Sent to Codex desktop${result.chatTitle ? `: ${result.chatTitle}` : ""}.`,
    speak: false,
  });
...
async function dispatchNow(command, route) {
  if (route.mode === WINDOWS_UI_MODE) {
    try {
      await appendBridgeEvent(command, {
        level: "system",
        text: "Sending through the visible Codex desktop window.",
        speak: false,
      });
